...
let adapter: ReturnType<typeof mongoAdapter>;

beforeAll(async () => {
  mongod = await MongoMemoryServer.create();
  adapter = mongoAdapter({ uri: mongod.getUri(), dbName: "easy-auth-test" });
}, 30_000);
...
describe("MongoDB StorageAdapter", () => {
  ...
  it("getOTP returns null for expired OTPs", async () => {
    // TTL index on MongoDB fires asynchronously, so we test via expiresAt filter.
    // We set a past expiry directly to verify the query filters it out.
  ...
  it("getLockout returns null for past lockout", async () => {
