...
function generateTOTPSecret() {
  const bytes = crypto3.randomBytes(20);
  let result = "";
  let buffer = 0;
  let bitsLeft = 0;
...
function computeTOTP(secret, counter) {
  ...
  const hmac = crypto3.createHmac(TOTP_ALGORITHM, key).update(counterBuf).digest();
...
function verifyTOTPCode(secret, inputCode) {
  ...
  for (let delta = -TOTP_WINDOW; delta <= TOTP_WINDOW; delta++) {
    ...
    if (expectedBuf.length === inputBuf.length && crypto3.timingSafeEqual(expectedBuf, inputBuf)) {
      return true;
    }
  }
  return false;
